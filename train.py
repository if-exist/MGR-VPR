...
args.features_dim = model.module.features_dim if isinstance(model, torch.nn.DataParallel) else model.features_dim
...
#### Getting GSVCities
train_dataset = get_GSVCities(args)
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=len(ds)*args.lr_step_epochs, gamma=args.lr_mult, last_epoch=-1)
